...
The source waveform is a multi-stem mixture, while each target waveform is generated by deterministic stem-level arithmetic before being encoded into discrete codec tokens. The edit instruction is encoded by MusicGen’s text encoder; LoRA provides parameter-efficient fine-tuning while the pretrained base weights remain frozen. Preservation is evaluated on content outside the edited stem, and current adherence, preservation, and quality values are explicitly proxy metrics rather than claims of state-of-the-art semantic evaluation.
